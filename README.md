...
- Notice that for convex problems any local optimum is also a global optimum. However, in non-convex problems this is might not be the case. For example, incorporating the eigenvalue centrality in the optimization problem further complexities, with respect to the convergence of an appropriate algirithm, occur which we will need to check. 
