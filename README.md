...
- Lastly, we point-out that the framework proposed in the studies of Katsouris (2021a, 2021b), discusses some important ideas with applications to other related fields as well. Specifically, the proposed algorithmic procedure of node centrality penalization (e.g., in the form of node removal), for the purpose of reducing the dependency in the graph without changing its main structure is also related to the so-called persistent data structure as well as to persistent homology from the fields of computational topology and topological data analysis, which indeed provides the link with our discussion regarding the importance of understanding some important aspects related to computational econometrics (see, Section [B] of this teaching page).    
